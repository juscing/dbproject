...
if(isset($_GET['actor']) && isset($_SESSION['user'])) {
	
	$user = $_SESSION['user'];
	$id = trim($_GET['actor']);
	
	$db_connection = DbUtil::loginConnection();
	if ($stmt = $db_connection->prepare("DELETE FROM Favorite_Actors WHERE username=? AND actor_id=?")) {
		...
		if($stmt->affected_rows === 0) {
			...
			if ($stmt2->prepare("INSERT INTO Favorite_Actors VALUES(?,?)")) {
